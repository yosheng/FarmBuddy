...
public class KernelManagerTest
{
    ...
    {
        var result = await _kernelManager.GetChatMessageContentAsync("417370","你是誰");
    ...
    {
        var result = await _kernelManager.GetChatMessageContentAsync("417370","請你告訴我雲林縣土庫鎮中水稻這兩天的天氣適合收成嗎?");
        _testOutputHelper.WriteLine(result.Content);
        Assert.NotNull(result);
    }
}